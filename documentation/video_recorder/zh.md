...
* **autoConfigAppAudioSession**
  是否自动配置应用音频会话
  默认为 `true`，会自动配置 SharedAudioSession 以优化录制 
  如果设置为 `false`，你的脚本需要负责选择合适的音频会话设置并调用相关的 `SharedAudioSession` API，录制可能会失败
...
重置录制器状态，用于开始新一轮录制。如果传入新设置，会覆盖当前设置，可用于切换摄像头等。
