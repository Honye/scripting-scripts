...
export const zh: Strings = {
  ...
  searching: '搜索中…',
  searchResults: '搜索结果',
  searchResultsNote: '名称与价格来自该地区。点某一项可在所有地区之间比价。',
  searchEmpty: '该地区没有找到',
  appLookupInvalid: '没能从这段文字里识别出 App ID',
  lookupAction: '查询',
  compareRegions: '地区',
  fxBase: '换算为',
  fxRatesFrom: (date) => `≈ 价格按 Frankfurter ${date} 的汇率换算，仅供参考 —— 实际按各地区自身币种扣款。`,
  fxUnavailable: '汇率获取失败',
  iapAutoLoad: '自动加载全部内购价格',
  iapAutoLoadNote: '每个地区的内购价格都要抓取完整商品页（约 700KB）。建议在 Wi‑Fi 下开启 —— 置顶地区无论开关都会自动加载。',
  loadedCount: (done, total) => `已加载 ${done} / ${total} 个地区`,
  notAvailableIn: (count) => `共 ${count} 个地区未上架`,
  storePrice: '售价',
  freePrice: '免费',
  inAppPurchases: '内购项目',
  loadIAP: '加载内购价格',
